							...
							<section class="pv-top" class:pv-top--bg-mode={effectiveBannerCss && bannerMode === 'background'} style="--container-opacity: {containerOpacity};">
								...
								{#if socialTwitch || socialYoutube || socialDiscord || socialTwitter || socialBluesky || socialInstagram || socialSpeedruncom || socialSteam || existingApprovedOther.length > 0}
									<div class="pv-socials">
										...
										{#if socialYoutube}<span class="pv-link"><span class="pv-link__icon"><Youtube size={12} /></span> YouTube</span>{/if}
										...
										{#if socialBluesky}<span class="pv-link"><span class="pv-link__icon"><Bird size={12} /></span> Bluesky</span>{/if}
										{#if socialInstagram}<span class="pv-link"><span class="pv-link__icon"><Camera size={12} /></span> Instagram</span>{/if}
										{#if socialSpeedruncom}<span class="pv-link"><span class="pv-link__icon"><Timer size={12} /></span> Speedrun.com</span>{/if}
						...
						<div class="form-row">
							<div class="fg fg--flex">
								...
								<div class="country-combobox-wrap">
									<Combobox.Root class="country-combobox" bind:inputValue={locationSearch} onInputValueChange={(v: string) => { locationSearchText = v; }} onValueChange={(v: string) => handleLocationSelect(v)} onOpenChange={(o: boolean) => { if (!o) locationSearchText = ''; }}>
							...
							<div class="fg fg--flex">
								...
								<div class="country-combobox-wrap">
									<Combobox.Root class="country-combobox" bind:inputValue={representingSearch} onInputValueChange={(v: string) => { representingSearchText = v; }} onValueChange={(v: string) => handleRepresentingSelect(v)} onOpenChange={(o: boolean) => { if (!o) representingSearchText = ''; }}>
...
<style>
	...
	/* Sticky header: preview + tabs */
	.edit-sticky-header {
		position: sticky; top: calc(4rem - 8px); z-index: 150;

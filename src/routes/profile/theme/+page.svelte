<script lang="ts">
	...
	// Preview-only info (read from profile, not editable here)
	...
	function getCurrentSnapshot(): string {
		return JSON.stringify({
			displayName, pronouns, location, representing, bio, statusMessage,
			avatarUrl, bannerUrl, bannerGradient, bannerIsGradient,
			bannerSize, bannerPosition, bannerOpacity, bannerMode,
			socialTwitch, socialYoutube, socialDiscord, socialTwitter,
			socialBluesky, socialInstagram, socialSpeedruncom, socialSteam,
			otherLinks, hideActivity, goals, highlights
		});
	}

	const hasUnsavedChanges = $derived(lastSavedSnapshot !== '' && getCurrentSnapshot() !== lastSavedSnapshot);
		...
		<div class="edit-page">
			...
			{:else}
								...
								<div class="preview-info">
									...
									{#if locC || repC}
										<p class="preview-location muted">
											{#if locC}
												<img class="flag-img" src="https://flagcdn.com/w40/{locC.code.toLowerCase()}.png" alt="{locC.name} flag" width="16" height="12" />
												{locC.name}
											{/if}
											{#if repC && repC.code !== locC?.code}
												<span class="preview-representing">
													{#if locC}·{/if} Representing
													<img class="flag-img" src="https://flagcdn.com/w40/{repC.code.toLowerCase()}.png" alt="{repC.name} flag" width="16" height="12" />
													{repC.name}
												</span>
											{/if}
										</p>
									{/if}
									{#if memberSince}
										<p class="preview-member-since muted">🗓️ Member since {formatDate(memberSince)}</p>
									{/if}
								</div>
							</div>
						</div>
							...
							<div class="banner-opts">
								<div class="banner-opt-row">
									...
									<div class="banner-opt-btns">
										{#each [['above','⬆️ Above Profile'],['background','🌅 Profile Background']] as [val,lbl]}
				...
				<div class="form-actions">
					{#if bannedTermsWarning}
						<div class="alert alert--error" style="margin-right: auto;">{bannedTermsWarning}</div>
					{/if}
...
<style>
	...
	.edit-sticky-header {
		...
		background: var(--bg); padding-top: 0.5rem; padding-bottom: 0;
	...
	.preview-info { display: flex; flex-direction: column; gap: 0.15rem; min-width: 0; }
	.preview-name { font-size: 1.1rem; font-weight: 700; }
	.preview-pronouns { font-size: 0.8rem; }
	.preview-location { margin: 0; font-size: 0.75rem; display: flex; align-items: center; gap: 0.25rem; flex-wrap: wrap; }
	.preview-location .flag-img { vertical-align: middle; border-radius: 1px; }
	.preview-representing { opacity: 0.75; font-size: 0.7rem; }
	.preview-member-since { margin: 0; font-size: 0.7rem; }

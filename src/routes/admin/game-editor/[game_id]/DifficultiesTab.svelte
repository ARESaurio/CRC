					...
					<Collapsible.Root class="children-section">
						...
						{#each item.children || [] as child, ci}
							<div class="child-inline">
								...
								{#if canEdit}<button class="item-btn item-btn--danger" onclick={() => { item.children = (item.children || []).filter((_: any, j: number) => j !== ci); difficultiesData = [...difficultiesData]; }}><X size={14} /></button>{/if}
							</div>
						{/each}

...
/** Try to match an old freeform location string to a country code */
export function matchLocationToCode(location: string): string {
	if (!location) return '';
	...
	// Partial match
	const partial = COUNTRIES.find(c => c.name.toLowerCase().includes(lower) || lower.includes(c.name.toLowerCase()));
